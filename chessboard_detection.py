...
def tiles64(points):
    ...
    # find edge points
    for j in range(1, n):
        ar[0][j] = ar[1][j]-(ar[2][j]-ar[1][j])
    for j in range(1, n):
        ar[n][j] = ar[n-1][j]+(ar[n-1][j]-ar[n-2][j])
    for i in range(1, n):
        ar[i][0] = ar[i][1]-(ar[i][2]-ar[i][1])
    for i in range(1, n):
        ar[i][n] = ar[i][n-1]+(ar[i][n-1]-ar[i][n-2])

    # # add 4 corner points
    ar[0][0] = ar[0][1]-(ar[0][2]-ar[0][1])
    ar[n][n] = ar[n][n-1]-(ar[n][n-2]-ar[n][n-1])
